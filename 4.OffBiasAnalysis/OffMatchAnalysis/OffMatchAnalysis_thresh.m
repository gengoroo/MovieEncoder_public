% 識別解析
%clear;
pn_3Ch = 'C:\Users\gengoro\SynologyDrive\Behavior\選別動画\crop2_sel\Rot0.48\Crop2_sel_final_LAPTOP-25N14DPB_2-08-151109-2024_CaseConflict\_Latest_UnifyCC\PTZ[5x7_0x1]_off_v2\DeepLearn__sum_10x10Gri_80x80Pix_e_AllEn25_OffAna\CC_var_hyper_3ChStack';
...
%-----------------------------------------------------------------------------------------------------------------
%GC解析
pn_CC = 'C:\Users\gengoro\SynologyDrive\Behavior\選別動画\crop2_sel\Rot0.48\Crop2_sel_final_LAPTOP-25N14DPB_2-08-151109-2024_CaseConflict\_Latest_UnifyCC\PTZ[5x7_0x1]_off_v2\DeepLearn__sum_10x10Gri_80x80Pix_e_AllEn25_OffAna';
...
for id_data = 1:numel(CCones_var_stack)
    im_raw = reshape(mean(CCones_var_stack{id_data}(:,:,1,:,id_cut),4),size(CCones_var_stack{id_data},1),size(CCones_var_stack{id_data},2));
    %im = uint8(round(255*(im_raw - min(im_raw(:)))/(max(im_raw(:)) - min(im_raw(:)))));
    %level = graythresh(im);
    level = mean(im_raw(:));
    temp = ones(size(im_raw));
    temp(im_raw<level) = 0;
    maskCC(:,:,id_data) = temp;
end

Order_dOFF_trim = Order_dOFF.*repmat(mask,1,1,size(Order_dOFF,3));
yn_thre = input('Exclude grids of under average intensity y/n','s');
if yn_thre == 'y'
    Order_dOFF_trim = Order_dOFF_trim.*maskCC;
end
...
saveas(h_speaman,[pn_3Ch '\' h_speaman.Name '.fig'],'fig');
%-----------------------------------------------------------------------------------------------------------------
